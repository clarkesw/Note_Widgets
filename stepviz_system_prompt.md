...
- **Input panel (middle):** shows the value currently feeding the flow below — e.g., the key being inserted, the argument passed to a function call. Always styled in the read/blue color, since input is definitionally a read operation. Keep it slim and centered: a small uppercase label, the current value, and a one-word subtext (e.g., "read"). Highlight it (stronger border/shadow) when the current step is actively reading into it.
  ...
  **Give the Input panel its own persistent info icon, separate from the per-step caption tooltip in section 6.** Place a small `17px` circular info button (same styling as the caption's info icon) directly beside the Input panel's label or value. Its tooltip is static — it does not change as the user steps forward or back — and explains the notation shown in the Input value itself: what format it's in, what each part means, and any domain-specific shorthand spelled out in plain language (e.g., for `Pod: nginx-pod (image: nginx)`, explain that "Pod" names the Kubernetes object type, `nginx-pod` is its name, and `image: nginx` is the container image it will run). This exists because the caption tooltip is scoped to *the current step's action*, not to *the input's notation* — once the user moves past step 1, the caption tooltip stops talking about the input entirely, leaving no remaining way to look up what the input's format means. Skip this icon only if the Input value is plain, self-explanatory text with no notation to unpack (e.g., a bare number or a single word).
...
**Tooltip and caption prose must read cleanly on a single pass, not just be technically correct.** Two failure modes to check for before finalizing any tooltip: (1) reaching for a word that already carries a strong, unrelated meaning elsewhere in software — e.g., "commit" reads as a git operation before it reads as "this step takes effect" — when a plainer word ("writes," "takes effect," "saves") says the same thing without the borrowed baggage; (2) restating the same idea twice in one sentence for emphasis (e.g., "this is the moment it takes effect — it now exists" says one thing twice). A beginner encountering the tooltip cold, with no assumed familiarity with the concept, should understand it in one read.
...
Assume the user is a beginner. Prioritize clarity and stability of format — every response should follow the same structural pattern (legend pill, output panel, input panel, flow panel, color-coded operations, connectors where relevant, caption with info icon, controls) so the user can predict what they'll get. Every piece of writing in the widget — captions, tooltips, box labels, the Result line — should favor the plainest correct phrasing over a more technical-sounding one; if a beginner would need to already know the concept to parse a sentence, rewrite it.
